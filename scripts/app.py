    ...
    def run(self):
        ...
        if trn_losses:
            ...
            with SummaryWriter(runs_dir) as writer:
                r = self.args.round
                # ── round 단위 (x = round 번호) ──────────────────────────
                writer.add_scalar("rnd_avg/trn_loss_avg",       avg_trn_loss, r)
                writer.add_scalar("rnd_avg/val_loss_avg",       avg_val_loss, r)
                writer.add_scalar("rnd_prv/val_loss_avg",       prv_val,      r)
                writer.add_scalar("rnd_pst/val_loss_avg",       pst_val,      r)
                writer.add_scalar("rnd_prvpst/val_loss_avg",    prv_val,      r)
                writer.add_scalar("rnd_prvpst/val_loss_avg",    pst_val,      r)
                writer.add_scalar("rnd_prv/val_dice_avg",       prv_dice_avg, r)
                writer.add_scalar("rnd_pst/val_dice_avg",       pst_dice_avg, r)
                writer.add_scalar("rnd_prvpst/val_dice_avg",    prv_dice_avg, r)
                writer.add_scalar("rnd_prvpst/val_dice_avg",    pst_dice_avg, r)
                for name in lnam:
                    writer.add_scalar(f"rnd_prv/val_dice_{name}",    prv_dice[name], r)
                    writer.add_scalar(f"rnd_pst/val_dice_{name}",    pst_dice[name], r)
                    writer.add_scalar(f"rnd_prvpst/val_dice_{name}", prv_dice[name], r)
                    writer.add_scalar(f"rnd_prvpst/val_dice_{name}", pst_dice[name], r)
                # ── epoch 단위 (x = global epoch 번호) ───────────────────
                writer.add_scalar("ech_avg/val_loss_avg",       avg_val_loss, epoch_end)
                writer.add_scalar("ech_prv/val_loss_avg",       prv_val,      epoch_start)
                writer.add_scalar("ech_pst/val_loss_avg",       pst_val,      epoch_end)
                writer.add_scalar("ech_prvpst/val_loss_avg",    prv_val,      epoch_start)
                writer.add_scalar("ech_prvpst/val_loss_avg",    pst_val,      epoch_end)
                writer.add_scalar("ech_prv/val_dice_avg",       prv_dice_avg, epoch_start)
                writer.add_scalar("ech_pst/val_dice_avg",       pst_dice_avg, epoch_end)
                writer.add_scalar("ech_prvpst/val_dice_avg",    prv_dice_avg, epoch_start)
                writer.add_scalar("ech_prvpst/val_dice_avg",    pst_dice_avg, epoch_end)
                for name in lnam:
                    writer.add_scalar(f"ech_prv/val_dice_{name}",    prv_dice[name], epoch_start)
                    writer.add_scalar(f"ech_pst/val_dice_{name}",    pst_dice[name], epoch_end)
                    writer.add_scalar(f"ech_prvpst/val_dice_{name}", prv_dice[name], epoch_start)
                    writer.add_scalar(f"ech_prvpst/val_dice_{name}", pst_dice[name], epoch_end)
